...
export const SunGraph = observer(() => {
    const sunPath = useRef<SVGPathElement>(null);
    const sun = useRef<SVGGElement>(null);
    const locationStore = useLocation();

    const getCurrentTimePercent = useCallback(() => {
        const from = locationStore.getTimeInLocation().hour(6).minute(34);
        const to = locationStore.getTimeInLocation().hour(18).minute(15);
        const currentTime = locationStore.getTimeInLocation();
