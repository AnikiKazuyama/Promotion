...
import {
    FindCityByQueryParams, FindCityByQueryResponse, FindCityByCoordsParams
...
import getWeatherApiInstance from './WeatherApiInstance';

const requester = async <ReqResponse, ReqParams>(
    url: string,
    params: ReqParams
) => {
    const response = await getWeatherApiInstance().get<ReqResponse>(url, { params });
...
export const findCityByCityCoords = async (coords: FindCityByCoordsParams) => {
    const listOfCitySuggestions = await findCityByCoords(coords);
    const city = listOfCitySuggestions.list[0];
